...
class ProjSimpleConvertor {
  ...
  private get toProjection() {
    if (!this._toProjection) this._toProjection = ProjSimpleConvertor.getProjection(this.outSR);
...
export class ProjGeometryConvertor {
  ...
  constructor(
    ...
    if (p.inSR || p.outSR) this.convertor = new ProjSimpleConvertor({ inSR: p.inSR, outSR: p.outSR });
  }
  protected _convert<T extends ProjSupportType>(geom: T, opts: { convertor: ProjConvertorFunc }): T {
    function __convert<T extends ProjSupportType>(geom: T) {
    ...
    if (geom.type === 'Point') {
      ...
      const coordinates = polygon.coordinates.map((coord1) => {
        const resp = __convert({ type: 'LineString', coordinates: coord1 } as GeoJSON.LineString) as GeoJSON.LineString;
