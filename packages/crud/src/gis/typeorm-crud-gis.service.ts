...
import { CreateManyDto, CrudRequest, CrudRequestOptions, GetManyDefaultResponse } from '../interfaces';
import { ParsedRequestParams } from '../request-parse';
import { TypeOrmCrudService } from '../typeorm/typeorm-crud.service';
import { ProjGeometryConvertor, ProjSupportType, proj4Convertor } from './convertor/proj-convertor';
...
export class GISTypeOrmCrudService<T> extends TypeOrmCrudService<T> {
  ...
  protected async setAndWhereFilterGeo(builder: SelectQueryBuilder<T>, filterGeo: QueryFilterGeo, pAlias = this.alias) {
  ...
  protected async setAndWhereBBox(builder: SelectQueryBuilder<T>, bbox: Envelope, pAlias = this.alias) {
    const geoColumn = this.getGeometryColumn();
  ...
  protected async doGetMany(
    ...
    if (query.fields.length === 0 || query.fields.indexOf(this.getGeometryColumn().propertyName) > -1)
      this.convertVn2000ToOutSR(data, query.outSR);
    return result;
  }

  private convertInSRToVn2000(data: DeepPartial<T>[] | DeepPartial<T>, inSR: number) {
    const geoColumn = this.getGeometryColumn();

    if (!this.equalSrs(inSR)) {
      const convertor = new ProjGeometryConvertor({ inSR });
      if (Array.isArray(data))
        ...
        data[geoColumn.propertyName] = convertor.convert(data[geoColumn.propertyName]);
      }
    }
  }
  private convertVn2000ToOutSR(data: T[] | T, outSR: number) {
    ...
    if (!this.equalSrs(outSR)) {
      const convertor = new ProjGeometryConvertor({ outSR });
      if (Array.isArray(data))
        data.forEach((d, idx) => {
          const geo = d[geoColumn.propertyName];
          if (geo !== null && geo !== undefined) {
  ...
  async getOne(crud: CrudRequest) {
    ...
    if (crud.parsed.fields.length === 0 || crud.parsed.fields.indexOf(this.getGeometryColumn().propertyName) > -1)
  ...
  async createOne(req: CrudRequest, dto: DeepPartial<T>) {
    ...
    }
    {
      ...
      if (this.repo.metadata.primaryColumns.length > 1) {
        this.throwBadRequestException('Chưa hỗ trợ lớp có nhiều hơn một khóa chính');
      }
  ...
  private async getPrimaryCol() {
    ...
    try {
      const response = await this.repo.manager
        ...
        .select(`COLUMNPROPERTY(object_id(TABLE_SCHEMA+'.'+TABLE_NAME), COLUMN_NAME, 'IsIdentity')`, 'value')
      ...
      throw new BadRequestException('Không xác định được IDENTITY của PRIMARY KEY');
    }
    return primaryCol;
  }
  ...
  async createMany(req: CrudRequest, dto: CreateManyDto<DeepPartial<T>>) {
    ...
    if (this.isEsriClass(primaryCol)) {
      const promises = [];
      bulk.forEach((d: any) => {
        const promise = this.generateEsriObjectId().then((objectId) => (d[primaryCol.propertyName] = objectId));
  ...
  getGeometryColumn(): ColumnMetadata {
    return this.repo.metadata.columns.find(
      (column) => this.repo.metadata.connection.driver.spatialTypes.indexOf(column.type) !== -1,
    );
  }
