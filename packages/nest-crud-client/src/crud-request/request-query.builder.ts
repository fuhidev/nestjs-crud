...
export class RequestQueryBuilder<T = any> {
  ...
  for (const key in this.queryObject) {
   if (this.queryObject.hasOwnProperty(key)) {
    ...
    if (isArrayFull(this.queryObject[key])) {
     queryString += (this.queryObject[key] as string[])
      .map((val) => `${key}=${val}`)
      .join("&");
 ...
 private addJoin(join: QueryJoin, parentField?: string): string[] {
  ...
  const rootField: string = parentField
   ? join.field + "." + parentField
   : join.field + "";
  result.push(
   rootField + (isArrayFull(join.select) ? d + join.select!.join(ds) : "")
  );
 ...
 private setCondition(
  f: QueryFilter | Array<QueryFilter>,
  cond: "filter" | "or"
 ): void {
  if (!isNil(f)) {
   //@ts-ignore
